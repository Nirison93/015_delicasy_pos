        ...
        >
          <DialogPanel class="bg-zinc-900 border border-white/10 rounded-3xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col overflow-hidden">
            ...
            <div class="flex items-center justify-between px-7 py-5 bg-gradient-to-r from-zinc-900 via-zinc-800 to-zinc-900 border-b border-white/10 flex-shrink-0">
              <div class="flex items-center gap-4">
                <div class="w-11 h-11 flex items-center justify-center rounded-2xl bg-amber-500/20 ring-1 ring-amber-500/40">
                  <i class="ri-add-box-line text-amber-400 text-xl"></i>
                </div>
                <div>
                  <DialogTitle class="text-2xl font-bold text-white">Add Product</DialogTitle>
                  <p class="text-sm text-zinc-400 mt-0.5">Fill in the details to create a new product</p>
                </div>
              </div>
            ...
            <div class="overflow-y-auto flex-1 px-7 py-6">
              <form @submit.prevent="submit" id="product-create-form">
                <div class="space-y-5">
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p v-if="form.errors.category_id" class="mt-1 text-base text-red-400">{{ form.errors.category_id }}</p>
                    </div>
                    <div>
                      ...
                      <p v-if="form.errors.supplier_id" class="mt-1 text-base text-red-400">{{ form.errors.supplier_id }}</p>
                    </div>
                  </div>

                  <!-- Product Name + Dry Food -->
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p v-if="form.errors.name" class="mt-1 text-base text-red-400">{{ form.errors.name }}</p>
                    </div>
                    <div>
                      <label class="block text-base font-semibold text-zinc-300 mb-1.5">Dry Food</label>
                      <div class="h-12 flex items-center">
                        <label class="inline-flex items-center gap-3 cursor-pointer">
                          <div class="relative">
                            <input v-model="form.is_beverage" type="checkbox" class="sr-only peer" />
                            <div class="w-11 h-6 bg-zinc-700 peer-focus:outline-none rounded-full peer peer-checked:bg-amber-500 transition-all"></div>
                            <div class="absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-all peer-checked:translate-x-5"></div>
                          </div>
                          <span class="text-base text-zinc-300">Is this product a dry food?</span>
                        </label>
                      </div>
                      <p v-if="form.errors.is_beverage" class="mt-1 text-base text-red-400">{{ form.errors.is_beverage }}</p>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <p v-if="form.errors.parent_id" class="mt-1 text-base text-red-400">{{ form.errors.parent_id }}</p>
                    </div>
                    <div>
                      ...
                      <p v-if="form.errors.sub_id" class="mt-1 text-base text-red-400">{{ form.errors.sub_id }}</p>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="number" step="0.01" v-model="form.cost_price" required placeholder="0.00"
                        class="w-full h-12 px-4 text-base bg-zinc-800 border border-white/10 rounded-xl text-white placeholder-zinc-500 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition" />
                      <p v-if="form.errors.cost_price" class="mt-1 text-base text-red-400">{{ form.errors.cost_price }}</p>
                    </div>
                    <div>
                      <label class="block text-base font-semibold text-zinc-300 mb-1.5">Stock Quantity</label>
                      <input type="number" v-model="form.stock_quantity" required placeholder="0"
                        class="w-full h-12 px-4 text-base bg-zinc-800 border border-white/10 rounded-xl text-white placeholder-zinc-500 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition" />
                      <p v-if="form.errors.stock_quantity" class="mt-1 text-base text-red-400">{{ form.errors.stock_quantity }}</p>
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <input type="text" v-model="form.selling_price" required placeholder="0.00" @blur="updateDiscountedPrice"
                        class="w-full h-12 px-4 text-base bg-zinc-800 border border-white/10 rounded-xl text-white placeholder-zinc-500 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition" />
                      <p v-if="form.errors.selling_price" class="mt-1 text-base text-red-400">{{ form.errors.selling_price }}</p>
                    </div>
                    <div>
                      <label class="block text-base font-semibold text-zinc-300 mb-1.5">Discount (%)</label>
                      <input type="text" v-model="form.discount" @blur="updateDiscountedPrice" placeholder="0"
                        class="w-full h-12 px-4 text-base bg-zinc-800 border border-white/10 rounded-xl text-white placeholder-zinc-500 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/30 transition" />
                    </div>
                    <div>
                  ...
                  <div>
                    ...
                    <textarea v-model="form.description" placeholder="Enter product description..." rows="2"
                    ...
                    <p v-if="form.errors.description" class="mt-1 text-base text-red-400">{{ form.errors.description }}</p>
                  </div>
                  ...
                  <div class="p-4 bg-zinc-800/50 border border-white/10 rounded-2xl">
                    ...
                    <p v-if="form.errors.image" class="mt-1 text-base text-red-400">{{ form.errors.image }}</p>
                  </div>
            ...
            <div class="flex items-center justify-end gap-3 px-7 py-5 bg-gradient-to-r from-zinc-900 via-zinc-800 to-zinc-900 border-t border-white/10 flex-shrink-0">
              ...
              <button type="submit" form="product-create-form" :disabled="form.processing"
                ...
                <i v-else class="ri-add-line text-lg"></i>
                Save Product
              </button>
            </div>
