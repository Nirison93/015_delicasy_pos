    ...
    <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 py-6 space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-2xl font-bold text-slate-800 tracking-wide uppercase">Bar Sales Report</h1>
            <p class="text-sm text-slate-400 font-medium mt-0.5">
              {{ dateRangeLabel }}
              <span v-if="barCategories.length" class="ml-2 inline-flex flex-wrap gap-1">
                <span v-for="c in barCategories" :key="c" class="px-2 py-0.5 text-xs font-semibold bg-rose-50 text-rose-600 ring-1 ring-rose-200 rounded-full">{{ c }}</span>
              </span>
            </p>
          </div>
        </div>
        ...
        <div class="flex flex-wrap items-center gap-3">
          <div class="relative">
            <button @click="showQuickFilter = !showQuickFilter"
                    class="h-12 px-5 inline-flex items-center gap-2 text-base font-semibold text-indigo-700 bg-indigo-50 ring-1 ring-indigo-200 rounded-xl hover:bg-indigo-100 transition select-none">
              <i class="ri-calendar-2-line text-lg"></i> Quick Filter
              <i :class="showQuickFilter ? 'ri-arrow-up-s-line' : 'ri-arrow-down-s-line'" class="text-lg"></i>
            </button>
            <div v-if="showQuickFilter" class="fixed inset-0 z-40" @click="showQuickFilter = false"></div>
            <ul v-if="showQuickFilter" class="absolute left-0 top-full mt-2 z-50 w-52 bg-white rounded-xl ring-1 ring-slate-200 shadow-xl overflow-hidden py-1">
              <li v-for="qf in quickFilters" :key="qf.key">
                <button @click="applyQuick(qf.key)" class="w-full text-left px-5 py-3 text-base font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 transition">{{ qf.label }}</button>
              </li>
            </ul>
          </div>
          <input v-model="startDate" type="date" class="h-12 px-4 text-base text-slate-700 bg-white ring-1 ring-slate-200 border-0 rounded-xl focus:ring-2 focus:ring-blue-400 transition" />
          <span class="text-base text-slate-400 font-semibold">to</span>
          <input v-model="endDate" type="date" class="h-12 px-4 text-base text-slate-700 bg-white ring-1 ring-slate-200 border-0 rounded-xl focus:ring-2 focus:ring-blue-400 transition" />
          <button @click="filterData" class="h-12 px-6 text-base font-semibold text-white bg-slate-800 rounded-xl hover:bg-slate-700 active:scale-95 transition">
            <i class="ri-filter-3-line mr-1.5 text-lg"></i> Filter
          </button>
          <Link href="/reports/bar-sales-report" class="h-12 px-6 flex items-center text-base font-semibold text-slate-600 bg-white ring-1 ring-slate-200 rounded-xl hover:bg-slate-100 transition">
            <i class="ri-refresh-line mr-1.5 text-lg"></i> Reset
          </Link>
        </div>
      </div>

      <!-- Empty state when no bar categories configured -->
      <div v-if="!barCategories.length" class="bg-amber-50 ring-1 ring-amber-200 rounded-2xl p-6 flex items-center gap-4">
        <i class="ri-information-line text-3xl text-amber-500"></i>
        <div>
          <p class="font-bold text-amber-800">No bar categories found</p>
          <p class="text-sm text-amber-600 mt-0.5">Create categories with <code class="font-mono bg-amber-100 px-1 rounded">category_type = 'bar'</code> or with "bar" in the name to populate this report.</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
        <div class="rounded-2xl bg-gradient-to-br from-rose-500 to-red-700 p-4 flex flex-col gap-1 shadow-md">
          <p class="text-xs font-semibold text-white/70 uppercase tracking-wider">Products Sold</p>
        ...
        <div class="rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-4 flex flex-col gap-1 shadow-md">
          <p class="text-xs font-semibold text-white/70 uppercase tracking-wider">Total Qty</p>
          <p class="text-2xl font-bold text-white">{{ grandQty.toLocaleString() }}</p>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl ring-1 ring-slate-200 shadow-sm p-5 flex flex-col">
          <h2 class="text-base font-bold text-slate-800 mb-4 flex items-center gap-2">
            <i class="ri-pie-chart-2-line text-rose-500"></i> Top Bar Products
          </h2>
        ...
        <div class="lg:col-span-3 bg-white rounded-2xl ring-1 ring-slate-200 shadow-sm p-5">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-base font-bold text-slate-800 flex items-center gap-2">
              <i class="ri-table-2 text-rose-500"></i> Bar Product Breakdown
            ...
            <table class="w-full text-sm text-slate-700 bg-white">
              ...
              <tbody class="divide-y divide-slate-100">
                <tr v-for="(r, i) in (rows.data ?? rows)" :key="i" class="hover:bg-slate-50 transition">
                  ...
                  <td class="p-3 font-semibold">{{ r.product }}</td>
                  <td class="p-3">
                    <span class="px-2 py-0.5 rounded-full text-xs font-semibold bg-rose-50 text-rose-600 ring-1 ring-rose-200">{{ r.category }}</span>
                  </td>
                  <td class="p-3 text-right">{{ r.qty.toLocaleString() }}</td>
                  ...
                  <td class="p-3 text-right">
                    <span class="inline-flex items-center gap-1">
                      <span class="w-14 bg-slate-100 rounded-full h-1.5 overflow-hidden">
                        <span class="block h-1.5 bg-rose-500 rounded-full" :style="{ width: share(r.total) + '%' }"></span>
                      </span>
                      {{ share(r.total).toFixed(1) }}%
                    </span>
                  </td>
                </tr>
                <tr v-if="!rowsData.length">
                  <td colspan="6" class="p-8 text-center text-slate-400 font-medium">No bar sales data for the selected period.</td>
                </tr>
              </tbody>
              <tfoot class="bg-slate-50 font-bold text-sm border-t-2 border-slate-200">
                <tr>
                  <td class="p-3 text-right" colspan="3">Totals</td>
                  <td class="p-3 text-right">{{ grandQty.toLocaleString() }}</td>
...
const props = defineProps({
  ...
  barCategories: { type: Array,  default: () => [] },
...
const grandQty   = computed(() => rowsData.value.reduce((s, r) => s + r.qty, 0));
...
const filterData = () => {
  router.get(route("reports.barSalesReport"), { start_date: startDate.value, end_date: endDate.value, page: 1 }, { preserveScroll: true });
};

const nextPage = () => {
  if (props.rows && props.rows.next_page_url) {
    router.get(
      route("reports.barSalesReport"),
      {
...
const COLORS = ["#ef4444","#f97316","#f59e0b","#10b981","#3b82f6","#8b5cf6","#ec4899","#14b8a6","#6366f1","#0ea5e9"];
const chartData = computed(() => ({
  labels: rowsData.value.slice(0, 10).map(r => r.product),
  datasets: [{ data: rowsData.value.slice(0, 10).map(r => r.total), backgroundColor: rowsData.value.slice(0, 10).map((_, i) => COLORS[i % COLORS.length]), borderWidth: 2, borderColor: "#fff" }],
}));
const chartOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: "bottom", labels: { boxWidth: 12, padding: 14 } }, tooltip: { callbacks: { label: (c) => ` LKR ${Number(c.raw).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}` } } } };
...
const downloadPDF = () => {
  ...
  doc.text("Bar Sales Report", 14, 17);
  ...
  doc.text("Bar Sales Report", 14, 32);
  doc.setFontSize(9); doc.setFont("helvetica","normal"); doc.setTextColor(100,116,139);
  doc.text(dateRangeLabel.value ? `Period: ${dateRangeLabel.value}` : "Period: All time", 14, 38);
  if (props.barCategories.length) doc.text(`Bar Categories: ${props.barCategories.join(", ")}`, 14, 43);
  doc.setDrawColor(226,232,240); doc.setLineWidth(0.4); doc.line(14, 46, pW - 14, 46);

  autoTable(doc, {
    startY: 51,
    margin: { left: 14, right: 14 },
    head: [["#","Product","Category","Qty","Revenue (LKR)","Share %"]],
    body: rowsData.value.map((r, i) => [i+1, r.product, r.category, r.qty.toLocaleString(), fmt(r.total), share(r.total).toFixed(1)+"%"]),
    foot: [["","Totals","", grandQty.value.toLocaleString(), fmt(grandTotal.value), "100%"]],
    theme: "grid",
    ...
    columnStyles: { 0:{cellWidth:10,halign:"center"}, 3:{halign:"right"}, 4:{halign:"right"}, 5:{halign:"right"} },
    didDrawPage: (d) => {
  ...
  doc.save(`BarSalesReport_${safe(dateRangeLabel.value)}.pdf`);
};

const downloadExcel = () => {
  const header = ["#","Product","Category","Qty","Revenue (LKR)","Share %"];
  const data = rowsData.value.map((r, i) => [i+1, r.product, r.category, r.qty, r.total, share(r.total).toFixed(1)+"%"]);
  data.push(["","Totals","", grandQty.value, grandTotal.value, "100%"]);
  const ws = XLSX.utils.aoa_to_sheet([header, ...data]);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Bar Sales");
  XLSX.writeFile(wb, `BarSalesReport_${safe(dateRangeLabel.value)}.xlsx`);
};
</script>
