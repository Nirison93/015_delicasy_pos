   ...
   public function fetchProducts(Request $request)
{
    ...
    $categoryType = $request->input('category_type'); // Add this
    ...
    $productsQuery = Product::with('category', 'color', 'size', 'supplier')
        ...
        // Filter by category_type through the category relationship
        ->when(in_array($categoryType, ['0', '1']), function ($queryBuilder) use ($categoryType) {
            $queryBuilder->whereHas('category', function ($catQuery) use ($categoryType) {
                $catQuery->where('category_type', $categoryType);
            });
        });
...
{
    $categoryType = $request->input('category_type'); // 0 for Food, 1 for Bar
    ...
    public function index(Request $request)
    {
        ...
        $products = $productsQuery->orderBy('created_at', 'desc')->paginate(18);
